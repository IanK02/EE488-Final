...
# --- Design region ---
dr_lx = 6.0           # design region length in x
dr_ly = 3.0           # design region width in y

# --- Run config ---
RUN_NAME     = "run7_explore"   # change for each run; controls output directory
...
global_res  = 16      # FDTD resolution (pixels/µm); Si slab = t_Si*global_res cells thick
dr_res      = global_res
opt_steps   = 4       # total NLopt iterations
...
skip_transmission_calibration = False
...
def _make_opt(source_band, col_idx):
    ...
    def objective(a_te, a_tm):
        # Partial trace contribution from this column (raw amplitudes, no normalization).
        # |Tr(U†U_meas)|² = |Σ_col overlap_col|²; we optimize the per-column |overlap|²
        # which provides the correct gradient direction without requiring both sims
        # to share a joint autograd computation.
        t0      = anp.conj(U_target[0, _col])
        t1      = anp.conj(U_target[1, _col])
        overlap = t0 * a_te[0] + t1 * a_tm[0]
...
stripe1 = 0.5 + 0.5 * np.sin(2 * np.pi * (XX + YY) / stripe_pitch)
...
stripe3 = 0.5 + 0.5 * np.sin(2 * np.pi * (XX - YY) / stripe_pitch)
...
_history_keys = ["J", "params", "grad", "beta", "U_meas", "Fh", "Tavg", "Uerr", "Penalty"]
...
def evaluate_and_grad(params_flat, beta, lambda_pen_val, step_num=None):
    """
    Run TE and TM adjoint solves; return (J, grad, info_dict).

    Args:
        params_flat   : raw optimizer parameters, shape (Npar,)
        beta          : tanh projection sharpness
        lambda_pen_val: fabrication penalty weight (0 = no penalty)
        step_num      : iteration number for log labels (None = 'eval')
    """
    rho = pre_process(params_flat, beta)

    # --- Forward + adjoint solves ---
    f_te, grad_te, a_te_TE, a_tm_TE = _run_adjoint(opt_TE, rho)
    f_tm, grad_tm, a_te_TM, a_tm_TM = _run_adjoint(opt_TM, rho)

    # --- Measured 2×2 transfer matrix (raw MEEP amplitudes) ---
    # After the two adjoint solves, normalize U_meas by REF_NORM
    if REF_NORM["TE"] is not None and REF_NORM["TM"] is not None:
        U_norm = np.array([
            [a_te_TE / REF_NORM["TE"], a_te_TM / REF_NORM["TM"]],
            [a_tm_TE / REF_NORM["TE"], a_tm_TM / REF_NORM["TM"]],
        ], dtype=complex)
    else:
        # Fallback: normalize each column to unit power
        # (loses relative phase between columns — use only if calibration unavailable)
        col_TE = np.array([a_te_TE, a_tm_TE])
        col_TM = np.array([a_te_TM, a_tm_TM])
        U_norm = np.stack([
            col_TE / (np.linalg.norm(col_TE) + 1e-30),
            col_TM / (np.linalg.norm(col_TM) + 1e-30),
        ], axis=1)
    
    # Use U_norm for fidelity, Uerr, and state verification
    trace_val = np.trace(U_target.conj().T @ U_norm)
    fidelity  = float(abs(trace_val)**2 / 4.0)
    U_meas    = U_norm   # everything downstream uses normalized matrix

    # --- Trace fidelity (gauge-invariant; peak = 1) ---
    trace_val = np.trace(U_target.conj().T @ U_meas)
    fidelity  = float(abs(trace_val)**2 / 4.0)

    # --- Physical transmission (requires calibration; diagnostic only) ---
    if REF_NORM["TE"] is not None and REF_NORM["TM"] is not None:
        col_TE_n = np.array([a_te_TE, a_tm_TE]) / REF_NORM["TE"]
        col_TM_n = np.array([a_te_TM, a_tm_TM]) / REF_NORM["TM"]
        transmission = float(0.5 * (np.linalg.norm(col_TE_n)**2
                                    + np.linalg.norm(col_TM_n)**2))
    else:
        transmission = float(np.linalg.norm(U_meas, "fro")**2 / 2.0)

    # --- Unitarity residual (diagnostic; ideal = 0) ---
    ...
    # --- Gradient assembly ---
    # 0.25 matches the 1/4 in |Tr|²/4; no per-column norm scaling (raw amplitudes).
    dJopt_drho    = 0.25 * (grad_te.ravel() + grad_tm.ravel())
    grad_optical  = pre_process_grad(params_flat, dJopt_drho, beta)
    ...
    if mp.am_master():
        print(f"[{tag}] J={J:.4f} | F={fidelity:.4f} | T={transmission:.4f} | "
              f"Uerr={Uerr:.4f} | pen={pen:.4f} (w={lambda_pen_val:.4f}) | β={beta:.1f}")
